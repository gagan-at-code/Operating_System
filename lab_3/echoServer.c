...
int main(int argc, char **argv) {
    int listenfd, connectedfd; /* listen descriptor */
...
int getlistenfd(char *port) {
    ...
    /* try to bind the first available address/port in the list. If we fail, try the next one */
...
/* From KERRISK 
    ...
    Read characters from 'fd' until a newline is encountered. If a newline character is not encountered in the first (n-1) bytes,
    then the excess characters are discarded. The returned string placed in 'buf' is null-terminated and includes the newline
    character if it was read in the first (n-1) bytes. The function return value is the number of bytes placed in buffer (which includes 
    the newline character if encountered but excludes the terminating null byte)
*/
ssize_t readLine(int fd, void *buffer, size_t n) {
